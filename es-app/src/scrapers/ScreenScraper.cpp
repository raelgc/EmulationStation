...
void ScreenScraperRequest::processGame(const pugi::xml_document& xmldoc, std::vector<ScraperSearchResult>& results)
{
	...
	if (game)
	{
		...
		std::string region = strToUpper(ssConfig.region).c_str();
		std::string language = strToUpper(ssConfig.language).c_str();
		...
		if (!description.empty()) {
			result.mdl.set("desc", strreplace(description, "&nbsp;", " "));
		}
		...
		if (_date.length() > 4)
		{
			result.mdl.setTime("releasedate", string_to_ptime(_date, "%Y-%m-%d"));
		} else if (_date.length() > 0)
		{
			result.mdl.setTime("releasedate", string_to_ptime(_date, "%Y"));
		}
		...
		if (media_list)
		{
			...
			if (art)
			{
				...
				result.imageUrl = strreplace(art.text().get(), " ", "%20");
