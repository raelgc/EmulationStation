...
{
	return boost::filesystem::path(getScrapersResouceDir() + "/" + file_name).generic_string();
}
...
std::string getScrapersResouceDir()
{
	...
	return boost::filesystem::path(getHomePath() + "/.emulationstation/" + SCRAPER_RESOURCES_DIR).generic_string();
